...
export function WhatsAppButton() {
  ...
  return (
    <a
      ...
      className="group fixed bottom-4 right-4 z-50 flex items-center gap-3 rounded-full bg-[#25D366] py-3 pl-4 pr-5 text-white shadow-[0_16px_40px_-16px_rgba(37,211,102,0.9)] transition-transform hover:scale-[1.03] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ink sm:bottom-6 sm:right-6"
    >
      <span className="relative flex">
        <span className="absolute inset-0 animate-ping rounded-full bg-white/40" aria-hidden="true" />
        <svg viewBox="0 0 24 24" aria-hidden="true" className="relative h-6 w-6 shrink-0 fill-current">
      ...
      <span className="text-left leading-tight">
        <span className="block text-[11px] uppercase tracking-[0.16em] text-white/85">Atendimento</span>
        <span className="block text-sm font-medium">{prettyPhone(cfg.phone) || "Fale conosco"}</span>
      </span>
    </a>
  );
}
