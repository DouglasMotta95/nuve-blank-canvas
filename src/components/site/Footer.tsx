      ...
      <div className="mx-auto w-full max-w-[1500px] px-4 py-10 sm:px-6 sm:py-12 xl:px-8 xl:py-14">
        <div className="grid gap-8 sm:grid-cols-2 xl:grid-cols-4 xl:gap-10">
          <div>
            ...
            <div className="mt-4 flex flex-col items-start gap-2">
              <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" aria-label="Abrir Instagram da NUVE" className="inline-flex items-center gap-2 text-sm text-ash transition-colors hover:text-ink">
                <Instagram className="size-4" /> Instagram
              </a>
              <a href={TIKTOK_URL} target="_blank" rel="noopener noreferrer" aria-label="Abrir TikTok da NUVE" className="inline-flex items-center gap-2 text-sm text-ash transition-colors hover:text-ink">
                <TikTokIcon className="size-4" /> TikTok
              </a>
            </div>

            <a href="mailto:nuveadvanced@gmail.com" className="mt-2 block break-all text-sm text-ash hover:text-ink">
          ...
          <div>
            ...
            >
              <input
                ...
                maxLength={160}
                className="min-w-0 flex-1 border border-input bg-ivory px-3 py-2 text-sm outline-none placeholder:text-ash/70"
              />
              <button type="submit" disabled={busy} className="min-h-10 bg-ink px-4 text-[11px] uppercase tracking-[0.18em] text-ivory disabled:opacity-50">
                Ok
              </button>
            </form>
          </div>
        ...
        <div className="mt-10 flex flex-col gap-4 border-t border-border pt-6 text-[11px] leading-relaxed text-ash xl:mt-12 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex flex-col items-start gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:gap-4">
            ...
            <Link to="/admin" aria-label="Acessar painel administrativo" className="inline-flex items-center gap-1 text-[9px] uppercase tracking-[0.16em] text-ash/60 transition-colors hover:text-ink">
